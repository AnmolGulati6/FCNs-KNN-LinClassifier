...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            trind = x_train[i]
            tind = x_test[j]
            computeSub = trind-tind
            exp = computeSub ** 2
            dists[i, j] = torch.sum(exp)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      ...
      dimensions = 1
      sizeOfDimensions = -1
      trind = x_train[i].view(sizeOfDimensions)
      tind = x_test.view(rows, sizeOfDimensions)
      sub = torch.sub(tind, trind)
      exp = sub ** raiseTo
      dists[i] = torch.sum(exp, dim = dimensions)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    rowste = num_test
    dimensions = 1
    trind = x_train.view(rowstr, sizeofdimensions)
    tind = x_test.view(rowste, sizeofdimensions)
    matrixMult = torch.mm(trind, tind.t())
    scalar = -2
    sumOne = torch.sum(tind**2, dim=dimensions)
    sumTwo = torch.sum(trind**2, dim=dimensions).view(-1, 1)
    sumSquares = sumOne+sumTwo
    eucladian = scalar * matrixMult + sumSquares
    dists = eucladian
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    dimensions = 0
    constant = 1
    sind = torch.argsort(dists, dim=dimensions)
    for i in range(num_test):
        inc = {}
        closestLabels = sind[:k, i]
        neighbours = y_train[closestLabels]
        for j in neighbours:
            inc[j.item()] = inc.get(j.item(),dimensions)+constant
        getmax = max(inc.values())
        maxLabels = [k for k, n in inc.items() if n==getmax]
        smallest = min(maxLabels)
        y_pred[i] = smallest
        ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
class KnnClassifier:
    ...
    def __init__(self, x_train: torch.Tensor, y_train: torch.Tensor):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        contigx = x_train.contiguous()
        contigy = y_train.contiguous()
        self.x_train = contigx
        self.y_train = contigy
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        contigx = x_test.contiguous()
        prdict = compute_distances_no_loops(self.x_train, contigx)
        y_test_pred = predict_labels(prdict, self.y_train, k=k)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    # Replace "pass" statement with your code
    dimensions = 0
    y_train_folds = torch.chunk(y_train,num_folds, dim=dimensions)
    x_train_folds = torch.chunk(x_train,num_folds, dim=dimensions)
    ...
    # Initialize k_to_accuracies with empty lists for each k_choice
    k_to_accuracies = {k: [] for k in k_choices}

    for k in k_choices:
        store = []
        for j in range(num_folds):
            storex, storey = x_train_folds[j], y_train_folds[j]
            x_trains = [x_train_folds[i] for i in range(num_folds) if i != j]
            storeXT = x_trains
            y_trains = [y_train_folds[i] for i in range(num_folds) if i != j]
            storeYT = y_trains
            xcat, ycat = torch.cat(storeXT, dim=dimensions), torch.cat(storeYT, dim=dimensions)
            classifier = KnnClassifier(xcat, ycat)
            storeClassifier = classifier.check_accuracy(storex, storey, k=k)
            accuracy = storeClassifier
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for i in k_to_accuracies.keys():
      high = statistics.mean(k_to_accuracies[i])
      if currBest > high:
        pass
      elif high > currBest:
        currBest = high
