...
def sample_batch(
    ...
    dimensions = 0
    labels = torch.randint(dimensions, num_train, (batch_size,))
    X_batch = X[labels]
    y_batch = y[labels]
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    dimensions = 1
    calc = X.mm(W)
    y_pred = torch.argmax(calc, dim=dimensions)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def softmax_loss_naive(
    ...
    for i in range(num_trains):
        currIndex = X[i]
        currScore = torch.matmul(currIndex, W)
        numStability = torch.max(currScore)
        move = currScore - numStability
        expM = torch.exp(move)
        compLoss = torch.log(torch.sum(expM))
        currLoss = -move[y[i]] + compLoss
        loss += currLoss
        softDenom = torch.sum(expM)
        res = expM / softDenom
        res[y[i]] -= 1
        dW += X[i].view(-1, 1) * res.view(1, -1)
    regu = torch.sum(W*W)
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    matr = torch.mm(X, W) 
    stab = torch.max(matr, dim=dimensions, keepdim=True).values
    storeScore = matr - stab
    expS = torch.exp(storeScore)
    denom = torch.sum(expS, dim=dimensions, keepdim=True)
    chance = expS / denom
    negLog = -torch.log(chance[range(num_train), y])
    loss = torch.sum(negLog)
    denomR = torch.sum(W * W)
    reg_loss = 1/2 * reg * denomR
    loss = loss/num_train + reg_loss
    chance[range(num_train), y] = chance[range(num_train), y] - dimensions
    dW = torch.mm(X.t(), chance)
    dW += reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-4, 1e-4, 5e-3, 2e-4] 
    regularization_strengths = [1e-4, 3e-4, 1e-6, 5e-3]  
